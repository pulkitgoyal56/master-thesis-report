...
We also compared these results with renderings of these sketches on ShapeGridWorld grids of different resolutions by registering them to a grid (see \secref{sec:sgw-registration} for more details).
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{images/p_sparse_rewards.png}
...
Although the greedier \texttt{best} aggregation function was more efficient in reaching the goal, for our semantics reward simulations, we mainly considered the \texttt{sum} aggregation function as it's more robust to noise due to its summation.
...
Additionally, the performance can highly depend on the choice of prefix and suffix (\figref{fig:prefix-suffix}).
...
More information of this analysis can be found in appendix \ref{sec:flatnet}.
...
Then, we searched for the semantics reward regularization hyperparameters that make CLIP less confident in the adversarial images while maintaining its inference for the truly semantically expressive images, i.e. decrease its specificity while maintaining its sensitivity.
...
Interesting creations from these simulations are shown in \secref{sec:gallery}.
...
\figref{fig:alpha-beta-trajectories} shows the effect of regularization strengths on the semantics entropy reward trajectories.
...
\figref{fig:alpha-beta-semantics} shows the heatmap of the mean cumulative rewards for different combinations of the regularization strengths and \figref{fig:alpha-beta-samples} shows some samples of the creations with different regularization strengths. For standard deviations, please see \figref{fig:alpha_beta-semantics_std_rair}.
...
\begin{figure}[h]
    ...
    \caption{Samples of creations with and without RaIR in ShapeGridWorld.}
