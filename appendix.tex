...
\figref{fig:flatnet-comparison} and \ref{fig:flatnet-comparison} show the reward trajectories of a regular and an entropy-regularized Flatnet model respectively.
These trajectories are random rollouts (similar to \figref{fig:random-rollout}) starting from a grid-registered image of the number zero.
Trajectories of all the models listed in \tabref{tab:flatnet-models} are neatly visualized and compared at \url{https://www.sharecanvas.io/p/flatnet-comparison}.
...
\footnotetext[1]{Note that the y-axis limits on the graphs in \figref{fig:flatnet-comparison-baseline} and \ref{fig:flatnet-comparison} are \([0, \max(H)]\), so the absolute values of the two graphs are not comparable, but the maximum entropy of the regularized version is \(\sim 1.5\) times higher than that of the unregularized version.}
...
\footnotetext[1]{The categories referred to in ``many categories'' are ``\texttt{house}'', ``\texttt{bird}'',``\texttt{tree}'',``\texttt{horse}'', ``\texttt{rabbit}'', ``\texttt{goat}'', ``\texttt{shirt}'', ``\texttt{chair}'', ``\texttt{swan}'', ``\texttt{camel}'', ``\texttt{teapot}'', ``\texttt{hammer}'', ``\texttt{boot}'', ``\texttt{key}'', ``\texttt{gun}'', ``\texttt{apple}'', ``\texttt{car}'', ``\texttt{guitar}'', ``\texttt{flower}'', and ``\texttt{heart}''.}
\footnotetext[2]{The categories referred to in ``few categories'' is the subset ``\texttt{bird}'', ``\texttt{goat}'', ``\texttt{shirt}'', ``\texttt{swan}'', ``\texttt{goose}'', ``\texttt{teapot}'', ``\texttt{gun}'', ``\texttt{apple}'', ``\texttt{car}'', ``\texttt{airplane}'', ``\texttt{guitar}'', and ``\texttt{flower}''.}
