...
We use the vision transformer variant (\emph{ViT-L/14}) with \(14 \times 14\) patch size for most of our experiments as it gave a good balance of performance and computational efficiency.
...
For objective analysis, we break the structure of a language prompt \(\bfc \in \cL\) representing a creative possibility \(c\) in an environment as ``\(\{\text{prefix}\}\{c\}\{\text{suffix}\}\)'', where ``prefix'' and ``suffix'' are optional context phrases.
...
This section describes the mathematical formulation of this semantics reward and the additional regularity reward that complements it.
...
Mathematically, in its simplest form, the semantics reward is formulated as the negative entropy of the likelihood of the predictions of the VLM for an image observation \(\bfo \in \cO\) over a set of \(n_c\) creative possibilities \(\bmc = \{ \bfc_1, \bfc_2, \ldots, \bfc_{n_c} \}\);
We call this the \emph{semantics-entropy reward},
\begin{equation}
    \label{eq:entropy-reward}
    r_{\text{semantics}}(\bfs, \bfo; \bmc, T) := -\cH(p(\bmc \mid \bfo; T)) = \sum_{\bfc_k \in \bmc} p(\bfc_k \mid \bfo; \bmc, T) \log p(\bfc_k \mid \bfo; \bmc, T).
\end{equation}
% 
...
\begin{equation}
    ...
    r_{\text{clip}}(\bfs, \bfo; \bfg) = S_c(\psi_I(\bfo), \psi_L(\bfg)).
\end{equation}
This captures the projection of the image observation embedding \(\bfo\) onto the direction spanned by the goal embedding \(\bfg\).

\cite{zest} developed an alternative method \emph{ZeST} (zero-shot task specification) in which they introduced \emph{delta embeddings} that use the difference in embeddings between the desired and initial/\emph{baseline} configurations, for both image and text inputs, to directionally remove irrelevant parts of the CLIP representation.
The goal-baseline regularized reward function in this case is the similarity between the delta embeddings,
\begin{equation}
    \label{eq:zest}
    r_{\text{zest}}(\bfs, \bfo; \bfo_b, \bfg, \bfg_b) = S_c((\psi_I(\bfo) - \psi_I(\bfo_b)), (\psi_L(\bfg) - \psi_L(\bfg_b))).
\end{equation}
where \(\bfg_b \in \cL\) is a natural language description of the initial/baseline environment image observation \(\bfo_b \in \cI\).
...
\begin{align}
    ...
    r_{\text{goal-reg}}(\bfs, \bfo; \bfg, \bfg_b, 0) &= 1 - \dfrac{1}{2}\ \norm{\psi_I(\bfo) - \psi_L(\bfg)}^2_2 = S_c(\psi_I(\bfo), \psi_L(\bfg)), \label{eq:vlmrm-0}
    \intertext{since \(\norm{\psi_I(\bfg_b)}_2 = \norm{\psi_L(\bfg)}_2 = 1\).
    On the other hand, for \(\gamma = 1\), the projection removes all components of \(\bfo\) orthogonal to ``\(\bfg - \bfg_b\)'' and only retains the components that align with the direction from the baseline to the goal.}
    r_{\text{goal-reg}}(\bfs, \bfo; \bfg, \bfg_b, 1) &= 1 - \dfrac{1}{2}\ \norm{\text{proj}_{V}\psi_I(\bfo) - \psi_L(\bfg)}^2_2\\
...
Let \(\bmp = \text{proj}_{V}\psi_I(\bfo)\), \(\bmo = \psi_I(\bfo),\ \bmg = \psi_L(\bfg),\ \bmg_b = \psi_L(\bfg_b)\) for brevity, then the VLM-RM reward can be simplified as
\vspace{-1.5pt}
\begin{align}
...
The regularized semantics-entropy reward is then given by,
\begin{align}
    r_{\text{semantics-reg}}(\bfs, \bfo; \bfo_b, \bmc, \bfc_b, \alpha, \beta, T)
...
\end{align}
\(\hat{p}\) is the likelihood distribution of the predictions of the VLM for the image observation \(\bfo\) over the creative possibilities \(\bmc\). For \(\bfc_k \in \bmc\),
\begin{equation}
...
\begin{equation}
    ...
    p(\cX) = \dfrac{m(\cX)}{\sum_{\cX' \in \phi(\bfs)} m(\cX')}.
\end{equation}
% 
